...
- gap: Compare 0x80018B08's draw-area semantics with the GPU state already captured by native submission. Then attribute the remaining side-character and top/bottom-arrow sprites inside the cached/model packet population rather than broadening the direct-pool boundary.
