...
- gap: Run tools/verify_cdc_phase_progress.py against the clean product built from recorded pin 17981527 when the sole game slot is granted; timeout is refusal and the tool stops only after positive LBA 17655. In the same serialized run, capture 0x800348A8 / 0x80027944 returns and require the oracle's guest-visible result 1 -> 0; the candidate's 5/5 distinct Pause response edges rule out the old controller-level coalescing mechanism but do not prove that result. Preserve the deterministic sector schedule; afterward make the shared watchdog count verified CD/FMV progress.
...
- evidence:
- where: game/render/
- gap: The landed phase machine's dirty candidate run crosses the former GetTN boundary, separates 5/5 Pause response pairs, and continues through later disc reads, but the guest-visible completion result and first frame remain unproven. Run the positive and strict completion verifiers on the clean pinned product when execution is authorized, record the next real divergence, then RE the loaded modules camera and submitters from game-state inputs before creating native producers.
