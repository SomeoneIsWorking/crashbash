...
- where: game/render/
- gap: Overall native 4:3 parity remains partial because large black angular holes still replace or mask broad regions of the purple starfield/background. Attribute that separate coverage defect to its exact source submitter/face family and first incorrect decode or rejection semantic before widescreen or interpolation. The verified letter-order fix does not authorize a skip, bias, or global visibility workaround.
- notes: Packet identity binds the formerly damaged pixel to accepted dark object `0x800A0C74` face 33/key 3312 and red object `0x801E18B0` face 50/key 3160. Both are opaque untextured Gouraud, so texture, CLUT, blend, and missing capture are excluded for that pixel. PID 3898998 verifies the frame-wide Authored resolver reproduces the retail red winner after the H-source fix and introduces no new letter occluder. The remaining angular background holes are visibly separate and are not explained by this two-face relation. A diagnostic RQ_WORLD/RQ_OM_2D_FG substitution was refused and reverted; no depth bias, skip, or title-local sorter is permitted.
