...
- gap: Run tools/verify_cdc_phase_progress.py against the clean product built from pinned 8611d756 when the sole game slot is granted; timeout is refusal and the tool stops only after positive LBA 17655. Then record the next real divergence and address completion coalescing only if the clean trace still shows one 0x8003F5F0 invocation dispatching bit-4 callback 0x80034040 (async=1) and bit-2 callback 0x8003400C (async=0) before the guest can observe result 1. Preserve the deterministic sector schedule; afterward make the shared watchdog count verified CD/FMV progress.
...
- gap:
- notes: No DAT or module bytes are tracked. The shared CDC phase machine is landed and candidate-run; issue 0009 remains open only for the clean pinned positive runtime gate. No game-local CD HLE or renderer fallback is permitted.
...
- evidence:
- where: game/render/
- gap: The landed phase machine's dirty candidate run crosses the former GetTN boundary and continues through later disc reads, but no first frame is proven. Run the positive verifier on the clean pinned product when execution is authorized, record the next real divergence, then RE the loaded modules camera and submitters from game-state inputs before creating native producers.
