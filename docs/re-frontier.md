...
- evidence:
- where: game/render/
- gap: RESOLVED for this frontier. The post-MENU no-present cause was measured, then fixed. A 200-frame product run is clean and the game reaches and runs its MENU scene: process state 0x8004E0B8 (the application shell, never re-entered because retail's own runner loops on it), app mode 0x8004E0DC set once to BOOT overlay base 0x80078C90, scene pointer 0x8009F658 advancing 0 -> 0x800A00DC -> 0x800B9524 inside the loaded MENU image, with update+present called every frame and two display fields delivered. Despite that, PSXPORT_DEBUG=rtpcaller emits NO histogram in 200 frames: the guest performs zero projection work, so there is no render ancestry to attribute yet and tools/verify_render_anchor_reach.py still has no real positive trace. Both blockers are now fixed (issues 0014 and 0012) and the port reaches the title screen: a 600-frame run exits 0, the guest submits 737,668 primitives, and tools/verify_render_anchor_reach.py passes its first real positive trace with 765,553 projection calls through anchors 0x800193A8 and 0x8001AF2C. The next step is the one this frontier item actually names: decompile that observed ancestry back to game-state camera and submitter inputs, then create native producers. The shipping native path presents black until they exist.
