...
- gap: Attribute the source model/material state behind the two side objective markers, which are bright yellow in retail but dim teal/gold natively. Indexed-frame interpolation remains refused until its source recipe is proven. `0x80018B08` is viewport/camera/render-list setup, not a drawable boundary.
