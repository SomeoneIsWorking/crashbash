...
| Model transform capture | `game/render/model_transform_capture.{h,cpp}` | The retained `0x8001965C` super owns guest composition; this wrapper copies its game-state affine/projection output from RAM or PSX scratchpad into the current snapshot |
...
| Fixed model recipe decode | `game/render/model_recipe_capture.{h,cpp}` | Source vertex/topology/material plus descriptor-run UV, texture-page, and CLUT decode stays independent of submission; animated families extend this decoder rather than inspecting guest packets |
...
| Native graphics producers | `game/render/native_model_producer.{h,cpp}` | One producer consumes copied snapshot recipes and submits fixed untextured/textured faces; ordering and unsupported frame families extend through proven source records |
